...
# Developers: Surbhi Sona (TING lab)
...
## This function reads count matrix from .txt file (add options for .csv in future) ##

get_count_mat<-function(sample_id,input_dir,data_dir=FALSE,tenX=TRUE,verbose=FALSE)
{
    if(tenX==TRUE)
    {
    ...
    { if(verbose)
      ...
      counts.mat<-Read10X_h5(filename = paste0(path,'filtered_feature_bc_matrix.h5'))
        ...
        fname<-sample_id
        sample_id<-gsub('_counts.txt','',fname)
        counts.mat<-read.table(paste0(input_dir,fname),header=TRUE,stringsAsFactors=FALSE)
    }
    return(count.mat)
}
...
{
  if(verbose)
    {catsum(PercentageFeatureSet(s.obj,pattern = "^MT-"))(paste0('Calculating MT gene % for ',sample_id),sep='\n')}

MT_sum<-sum(PercentageFeatureSet(s.obj,pattern = "^MT-"))
mt_sum<-sum(PercentageFeatureSet(s.obj,pattern = "^mt-"))

if((MT_sum==0) || (is.nan(MT_sum)))
...
## This function creates Seurat object from count matrix ##

create_seurat_obj_from_counts_data<-function(count.mat,sample_id)
{
    
s.obj<-CreateSeuratObject(counts=count.mat,names.field = 1:2, project = sample_id)
...
### Function to create Seurat obj ###

create_seurat_obj<-function(sample_id,input_dir,data_dir=FALSE,tenX=TRUE,verbose=FALSE)
{
 
 # Read count mat
 count.mat<-get_count_mat(sample_id,input_dir,data_dir=data_dir,tenX=tenX,verbose=verbose)

# Create seurat obj from counts
 s.obj<-create_seurat_obj_from_counts(count.mat,sample_id)
 return(s.obj)
...
create_seurat_obj_10X<-function(sample_id,input_dir,data_dir=FALSE,tenX=TRUE,verbose=FALSE)
{
 if(tenX==TRUE)
 {
 # Read data from cellranger output 
 ...
 { if(verbose)
   ...
   counts.mat<-Read10X_h5(filename = paste0(path,'filtered_feature_bc_matrix.h5'))
    ...
    counts.mat<-read.table(paste0(input_dir,sample_id),header=TRUE,stringsAsFactors=FALSE)
 }
# Create Seurat object
 if(verbose)
    {cat(paste0('Creating Seurat Object for ',sample_id),sep='\n')}
 if(length(grep('.txt',sample_id))!=0)
     {sample_id<-gsub('_counts.txt','',sample_id)}
 s.obj<-CreateSeuratObject(counts=counts.mat,min.cells = 0,names.field = 1:2, project = sample_id)
 s.obj<-add_mt_perc(s.obj,sample_id)
...
### Function to create seurat object for subset analysis ###

create_subset_obj<-function(obj_path,clusters='all',verbose=FALSE)
{
 # Read object
...
#Create Seurat object
obj.list<-mapply(create_seurat_obj_from_counts_data,sub.counts,sample.id,verbose=verbose)
...
### Function to add custom variable genes ###
## This function computes n highly variable genes for P0 and Pn features separately and then adds this combined list as variable features to peak x cell seurat obj. Different n can be specified for P0 and Pn features ##

# p.obj: Seurat peak x cell obj
# hvg_0: number of hvg to find amongst P0 features
# hvg_n: number of hvg to find amongst Pn features

find_P0_Pn_var_features<-function(p.obj,hvg_0,hvg_n,verbose=FALSE)
{
DefaultAssay(p.obj)<-'RNA'

# Split by P0 and Pn features
i0<-grep('P0',rownames(p.obj))
p0<-rownames(p.obj)[i0] # 2328
pn<-setdiff(rownames(p.obj),p0) # 180762

p0_obj<-subset(p.obj, features=p0)
pn_obj<-subset(p.obj,features=pn)

p0_obj<-FindVariableFeatures(p0_obj,selection.method = 'vst',
                     nfeatures=hvg_0,
                     verbose = verbose)
pn_obj<-FindVariableFeatures(pn_obj,selection.method = 'vst',
                             nfeatures=hvg_n,
                             verbose = verbose)

var_features<-c(p0_obj@assays$RNA@var.features,pn_obj@assays$RNA@var.features)

# Add variable features to p.obj
VariableFeatures(p.obj)<-var_features
return(p.obj)
}


### Function to split peak obj ###
## This function splits peak obj into subset consisting of P0 features and Pn features ##

# p.obj: Seurat peak x cell obj

split_peak_obj1<-function(p.obj)
 { DefaultAssay(p.obj)<-'RNA'
  
  # Split by P0 and Pn features
  i0<-grep('P0',rownames(p.obj))
  p0<-rownames(p.obj)[i0]
  pn<-setdiff(rownames(p.obj),p0)
  
  p0_obj<-subset(p.obj, features=p0)
  pn_obj<-subset(p.obj,features=pn)
  
  lst<-list(p0_obj,pn_obj)
  names(lst)<-c('p0','pn')
...
split_peak_obj<-function(p.obj,max_peaks=20)
 { DefaultAssay(p.obj)<-'RNA'
  ...
  # Remove genes that have too many peaks
  tu<-strsplit(pn,split=':') %>% sapply(.,'[[',1)
  gene<-strsplit(pn,split=':') %>% sapply(.,'[[',2)
  peak<-strsplit(pn,split=':') %>% sapply(.,'[[',3)
  
  pn_tab<-cbind(tu,gene,peak) %>% as.data.frame()
  
  pn_tab_count<-pn_tab %>% group_by(tu, gene) %>% tally()
  pn_tab_count<-pn_tab_count[order(pn_tab_count$n),]
  

  pn_tab_count_rem<-pn_tab_count[which(pn_tab_count$n>max_peaks),]
  rem<-which(pn_tab$tu %in% pn_tab_count_rem$tu)
  
  #since order of pn_tab$tu matches pn TU order
  ...
  p0_obj<-subset(p.obj, features=p0)
  #pn_obj<-subset(p.obj,features=pn)
  pn_obj<-subset(p.obj,features=pn_filtered)
  lst<-list(p0_obj,pn_obj)
  names(lst)<-c('p0','pn')
...
cca_batch_correction<-function(s.obj.list,project.name,anchors=2000,int.genes='',features='',norm='',verbose=FALSE)
{
  if(verbose){
...
# metadata - dataframe containing metadata (stored in the format: rows = sample and column = meta data)
# sample_col - column name in metadata table that specifies sample ID; if not specified, it searches for 'sample', 'orig.ident' and 'orig_ident'
# meta_col - specify which metadata col is to be added to seurat obj; if not specified, all except sample_col is added to the object

add_metadata<-function(s.obj,metadata,sample_col,meta_col)
{
 
  #Match obj metadata orig.ident to meta table sample column (if not specified, it is assumed there is a 'sample' column)
  if(missing(sample_col))
  {
    if(c('orig.ident') %in% colnames(metadata))
    {
      sample_col<-'orig.ident'
    }else if(c('orig_ident') %in% colnames(metadata))
    {
      sample_col<-'orig_ident'
    }else if(c('sample') %in% colnames(metadata))
    {
      sample_col<-'sample'
    }else
    {
      cat('Error: No sample column specified for metadata table')
    }
  }
  
  indx<- match(s.obj@meta.data$orig.ident,metadata[,sample_col])
  
  
  cols<-colnames(metadata)
  
  rem<-grep(sample_col,cols)
  cols<-cols[-rem]
  ...
  {
    s.obj[[cols[i]]]<- metadata[indx,cols[i]]
  }
  }else
  {
    s.obj[[meta_col]]<- metadata[indx,meta_col] 
  }
  ...
  {if( sum(s.obj@meta.data[[seurat_clusters]]==clusters[i])<4)
    {
     cat('Cluster',clusters[i],'has too few cells','\n')
   	  ...
   	  markers[[i]]<-FindMarkers(s.obj, ident.1= clusters[i])
    	  ...
    	  markers[[i]]<-markers[[i]] %>% filter(p_val_adj<0.05) %>% mutate(pct.diff=pct.1-pct.2) %>% arrange(desc(pct.diff),desc(avg_log2FC))
   #markers[[i]]<-markers[[i]][order(markers[[i]]["avg_log2FC"], decreasing=TRUE),]
    	  if(save)
