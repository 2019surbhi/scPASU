...
create_apa_inputs<-function(merged_counts,meta,c1,c2,c1_nm,c2_nm,out,plot_corr=FALSE)
{
  ...
  c2_rep_lst<-create_replicates(bc_dir,clus_name=c2_nm,peak_clus_mat=c2_peaks,nrep=2)
...
testAPA_sc<-function(stab,peak_mat,meanmat,peak_ref,a=a,b=b,adjust.var=NULL,min_peak,ncpu=20,cov=10)
{
...
testEdgeR <- function(myta,a,b)
{
  ...
  sp <- diffSpliceDGE(fit, coef=1, geneid="tu", exonid="pr")
...
callsig <- function(myta)
{
  ...
  myta$res$delta_sig <- ((abs(myta$res$b_minus_a_frac)>=0.1)&(myta$res$padj<0.0001))
  
  # P-value and fold change
  myta$res$perfc_sig <- with(myta$res,(padj<0.0001)&(abs(l2_b_over_a_frac)>log2(1.5))&((mean_a_frac>=0.05)|(mean_b_frac>=0.05)))
...
get_final_APA_tab<-function(tab,g1,g2)
{
  ...
  setnames(tab,"padj","dex_padj")
  #setnames(tab,"batchadj_delta_sig","delta_sig")
  setnames(tab,"perfc_sig","fracfc_sig")
