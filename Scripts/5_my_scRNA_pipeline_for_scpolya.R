...
# Developers: Surbhi Sona

source('/home/sonas/beegfs/my_scripts/pipeline/scRNA/my_seurat/my_scRNA_pipeline_functions_developing.R')

library(argparser)

### User inputs and parameters ###

parser<-arg_parser(name="my_scRNA_pipeline_developing.R",description="Ting lab Seurat pipeline for scRNA and subset analysis")
...
parser<-add_argument(
  ...
  short = '-T',
  flag=TRUE,
  help="Flag set to read 10X cellranger output else read count matrix from .txt file")
...
parser<-add_argument(
  ...
  type="character",
  default='',
  help="Enter the marker list .xlsx file with compartment markers in 3 columns")
...
parser<-add_argument(
  ...
  default='none',
  type='character',
  help="Enter txt filename containing variable genes. Defaault is none ")
...
parser<-add_argument(
  ...
  default='2000',
  type='character',
  help="Enter the number of highly variable genes to be used for clustering. Defulat = 2000. Enter separate hvf for P0 an dPn features separated by , for scpolya workflow")
...
parser<-add_argument(
  ...
  default=20,
  type='numeric',
  help="Enter the max number of peaks to consider for retaining a pn TU to be used for clustering. Defulat = 20")
...
short = '-n',
type='character',
default="KRT5,KRT7,KRT8,KRT14,KRT15,KRT17,KRT20,UPK2,UPK3A,UPK3B,UPK1A,UPK1B,EPCAM,PTPRC,TRAC,CD4,CD8A,CD68,CD79A,CD79B,LST1,TRAC,NKG7,KLRD1,TPSAB1,PECAM1,FLT1,DCN,ACTA2,MHY11,MKI67,TP63",
...
# Process hvg arguments
cat('Processing number of HVF argument \n')

hvg<-strsplit(args$hvg,split=',') %>% unlist() %>% as.numeric()
if(length(hvg)>1)
{  cat('More than 1 HVG specified \n')
    hvg0<-hvg[1]
    hvgn<-hvg[2]
    args$hvg<-hvgn
}else{
 cat('Only 1 HVG specified \n')
 args$hvg<-as.numeric(args$hvg)
 hvg0<-args$hvg
 hvgn<-args$hvg
}

cat('Processing HVG list argument')
...
if((args$clusters!="all") && (args$clusters!="none"))
{
 cat('Splittling clusters \n')
  ...
  if(length(paths.list)>1)
   {
    ...
    for(i in 1:length(paths.list))
    {
     ...
     s.list[[i]]<-lapply(s[[i]][1:n], create_seurat_obj_10X,input_dir=paths.list[i],args$data_dir, tenX=args$tenX,verbose=args$verbose)
    }
   ...
   rm(s)
    ...
    obj.list<-lapply(args$samples[1:n],create_seurat_obj_10X,input_dir=args$input_dir,args$data_dir, tenX= args$tenX, verbose=args$verbose)
    }
  ...
  { # [if-else block 1]: If object is specified then run subset analysis
   ...
   if(args$clusters=="none")
     { ## [if-else block 1d]: if clusters='none' then the object is entered to run rest of the pipeline skipping batch correction and the steps prior to it
	  ...
	  if((args$batch_correction=="cca-mnn") | (args$batch_correction=="none"))
           {	   
            assay<-'integrated'
            reduction<-'pca'
...
#saveRDS(obj.list,paste0(args$output_dir,'obj_list.Rds'))
...
#obj.list<-lapply(X=1:length(obj.list),
#                 function(x){find_P0_Pn_var_features(obj.list[[x]],
#                                                  hvg_0=hvg0,hvg_n=hvgn,
#
#                                                 verbose = TRUE)})

#var_features<-lapply(1:length(obj.list), function(x){obj.list[[x]]@assays$RNA@var.features}) %>%
#  unlist() %>% unique()
  
  lst<-lapply(obj.list,split_peak_obj1)
  #lst<-lapply(obj.list,split_peak_obj,max_peaks=args$pn_filter)
  p0_lst<-lapply(1:length(lst),function(x){lst[[x]]$p0})
  pn_lst<-lapply(1:length(lst),function(x){lst[[x]]$pn})
 
# Get P0 var features
 p0_lst<-lapply(p0_lst,FindVariableFeatures,selection.method = 'vst',
                               nfeatures=hvg0,
                               verbose = args$verbose)

f0<-SelectIntegrationFeatures(p0_lst,nfeatures=hvg0)

if(length(var_features)>1)
	{if(var_features!='none')
	  {fn<-var_features
	  }
	 }else{
		pn_lst<-lapply(pn_lst,FindVariableFeatures,selection.method = 'vst',nfeatures=hvgn,verbose = args$verbose)
	 	fn<-SelectIntegrationFeatures(pn_lst,nfeatures=hvgn)
		}
var_features<-c(f0,fn)
...
if(!dir.exists(paste0(args$output_dir,'variable_genes/P0/')))
 { dir.create(paste0(args$output_dir,'variable_genes/P0/'),
             recursive=TRUE)}

if(!dir.exists(paste0(args$output_dir,'variable_genes/Pn/')))
  {dir.create(paste0(args$output_dir,'variable_genes/Pn/'),
             recursive=TRUE)}
  
  out_path0<-paste0(args$output_dir,'variable_genes/P0/')
  out_pathn<-paste0(args$output_dir,'variable_genes/Pn/')

  lapply(X=1:length(p0_lst),function(x){get_var_genes(p0_lst[[x]],out_dir=out_path0,verbose=args$vebose)})
  varplots0<-lapply(p0_lst[1:length(p0_lst)],get_var_genes_plot,verbose=args$verbose)
  
  pdf(file=paste0(out_path0,args$file_prefix,'VariableGenePlots_all_samples_P0_sub.pdf'),paper='a4')
  print(varplots0)
  dev.off()
  
  lapply(X=1:length(pn_lst),function(x){get_var_genes(pn_lst[[x]],out_dir=out_pathn,verbose=args$vebose)})
  varplotsn<-lapply(pn_lst[1:length(pn_lst)],get_var_genes_plot,verbose=args$verbose)
  
  pdf(file=paste0(out_pathn,args$file_prefix,'VariableGenePlots_all_samples_P0_sub.pdf'),paper='a4')
  print(varplotsn)
  dev.off()
...
if(length(obj.list)>1)
 ...
 if(args$batch_correction=='cca-mnn')
 {
  ...
  obj.integrated<-cca_batch_correction(obj.list,project.name=args$file_prefix, anchors=args$hvg, int.genes=args$batch_genes,features=var_features,norm="LogNormalize", verbose=args$verbose)
}else if(args$batch_correction=='harmony')
   {
     if(args$verbose)
...
#Import metadata
  ...
  # Determine if it is xlsx or xls
  if(grepl('\\bxlsx\\b',args$meta_file))
  {
    metadata<-read_xlsx(args$meta_file) %>% as.data.frame()
  }else if(grepl('\\bxls\\b',args$meta_file)==TRUE)
  {
    metadata<-read_xls(args$meta_file) %>% as.data.frame()
  }else{ # If not xlsx or xls then use fread to read csv, tsv or txt formats
          metadata<- fread(args$meta_file,header = TRUE,stringsAsFactors = TRUE)
        }

obj.integrated<-add_metadata(obj.integrated,metadata)
}
...
#     {
#         var_features<-obj.integrated_RNA@assays$RNA@var.features
#     }else #Integrated assay exists and has >0 var.features
#        {
#          var_features<-obj.integrated_RNA@assays$integrated@var.features
#        }
...
#obj.integrated_RNA<-RunPCA(obj.integrated_RNA,assay='RNA',features=var_features)
...
# sample
customized_umap(obj.integrated, umap_cols=NULL,label=FALSE,title=NULL, group='orig.ident',split=NULL,dot=0.3,save=TRUE,out=out_dir,file_prefix=args$file_prefix)

if(args$meta_file!='')
{
 # The meta_file will already be read as metadata if args$meta_file!='' for adding metadata after sample intgration
 meta_cols<-colnames(metadata)
 for(i in 1:length(meta_cols))
 {
 customized_umap(obj.integrated, umap_cols=NULL,label=FALSE,title=NULL, group=meta_cols[i],split=NULL,dot=0.3,save=TRUE,out=out_dir,file_prefix=args$file_prefix)
 }
...
## (9f) Cluster annotation ###

# Read marker file
if(args$marker_file!='')
{

    m<-readxl::read_excel(args$marker_file)
    markers<-c(m$immune,m$stromal,m$urothelial)
    rem<-which(markers=='-')
    markers<-markers[-rem]

if(!dir.exists(paste0(args$output_dir,'cluster_annotation/')))
        {dir.create( paste0(args$output_dir,'cluster_annotation/'))}
    out<-paste0(args$output_dir,'cluster_annotation/')

dotplot_grids(m=markers, obj.integrated, out=out, run_name=args$file_prefix,
               assay_d='RNA', split=NULL, col=c("lightgrey","blue"),
               text_col='black', h=8.5,w=11, group='seurat_clusters')
}else
  {
    cat('cannot print dotplot wihtout marker list file \n')
  }
